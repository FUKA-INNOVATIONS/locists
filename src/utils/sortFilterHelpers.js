export const sortLatest = ( arr ) => {
  return arr.sort( function( a, b ) {
    return Number( new Date( b.time_added ) ) -
      Number( new Date( a.time_added ) )
  } )
}

export const sortMostAttendees = ( arr ) => {
  return arr.sort( function( a, b ) {
    return Number( ( b.description.attendeesCount ) -
      Number( a.description.attendeesCount ) )
  } )
}
...
export const sortMostCommented = ( arr ) => {
  return arr.sort( function( a, b ) {
    return Number(
      ( b.description.commentsCount ) - Number( a.description.commentsCount ) )
  } )
}
...
export const sortMostLikes = ( arr ) => {
  return arr.sort( function( a, b ) {
    return Number(
      ( b.description.likesCount ) - Number( a.description.likesCount ) )
  } )
}
...
export const initSearchNames = ( arr, setArr ) => {
  const names = []
  arr.map( item => names.push( item.description.name ||
    `no-name-${ Math.floor( Math.random() * ( 999 - 100 + 1 ) + 100 ) }` ) )
